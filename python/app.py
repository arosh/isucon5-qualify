...
def get_index():
    ...
    entries = db_fetchall(query, user_id)
    ...
    sql = """
        ...
        LIMIT 10"""
    comments_for_me = db_fetchall(sql, user_id)

    sql = "SELECT another FROM relations WHERE one = %s"
    friend_ids = list(r["another"] for r in db_fetchall(sql, user_id))
    
    entries_of_friends = []
    with db().cursor() as cursor:
        sql = """
        SELECT entries.body AS body,
            entries.id AS id,
            entries.created_at AS created_at,
            users.account_name AS owner_account_name,
            users.nick_name AS owner_nick_name
        FROM entries USE INDEX(created_at)
        JOIN users ON users.id = entries.user_id
        WHERE entries.user_id IN ({0})
        ORDER BY created_at DESC
        LIMIT 10""".format(",".join(["%s"] * len(friend_ids)))
    ...
    SELECT comments.user_id AS user_id,
        ...
        comment_owner.account_name AS comment_owner_account_name,
        comment_owner.nick_name AS comment_owner_nick_name,
        entry_owner.account_name AS entry_owner_account_name,
        entry_owner.nick_name AS entry_owner_nick_name
    FROM comments USE INDEX(created_at)
    JOIN entries ON entries.id = comments.entry_id
    JOIN users comment_owner ON comment_owner.id = comments.user_id
    JOIN users entry_owner ON entry_owner.id = entries.user_id
    WHERE comments.user_id IN ({0})
    AND (entries.private != 1 OR entries.user_id = %s OR entries.user_id IN ({0}))
    ORDER BY comments.created_at DESC
    LIMIT 10""".format(",".join(["%s"] * len(friend_ids)))

    args = []
    args.extend(friend_ids)
    args.append(user_id)
    args.extend(friend_ids)
    comments_of_friends = db_fetchall(sql, *args)
    
    num_friends = db_fetchone("SELECT COUNT(*) AS c FROM relations WHERE one = %s", user_id)["c"]
    ...
    sql = """SELECT
             ...
             FROM footprints
             JOIN users ON users.id = footprints.owner_id
             WHERE user_id = %s
             GROUP BY user_id, owner_id, DATE(created_at)
             ORDER BY updated DESC
             LIMIT 10"""
    footprints = db_fetchall(sql, user_id)
...
def get_entries(account_name):
    ...
    if permitted(owner["id"]):
        ...
        SELECT entries.id AS id,
            entries.private AS private,
            entries.body AS body,
            entries.created_at AS created_at,
            COUNT(entries.id) AS comment_count
        FROM entries
        LEFT OUTER JOIN comments ON comments.entry_id = entries.id
        ...
        ORDER BY entries.created_at DESC
        LIMIT 20"""
    entries = db_fetchall(sql, owner["id"])
    for entry in entries:
        entry["is_private"] = entry["private"] == 1
        entry["title"], entry["content"] = entry["body"].split("\n", 1)
    mark_footprint(owner["id"])
    return bottle.template("entries", {
        "owner": get_user(owner["id"]),
        "entries": entries, 
        "myself": get_session_user_id() == owner["id"],
    })
...
def post_comment(entry_id):
    ...
    db_execute(query, entry["id"], get_session_user_id(), bottle.request.forms.getunicode("comment"))
...
def get_footprints():
    ...
    sql = """SELECT
             ...
             ORDER BY updated DESC
             LIMIT 50"""
    footprints = db_fetchall(sql, get_session_user_id())
...
def get_friends():
    authenticated()
    sql = """
        SELECT relations.created_at AS created_at,
            users.account_name AS friend_account_name,
            users.nick_name AS friend_nick_name
        FROM relations
        JOIN users ON users.id = relations.another
        WHERE one = %s
        ORDER BY created_at DESC"""
    friends = db_fetchall(sql, get_session_user_id())
...
def post_friends(account_name):
    ...
    if not is_friend_account(account_name):
        ...
        db_execute("INSERT INTO relations (one, another) VALUES (%s, %s), (%s, %s)",
                   get_session_user_id(), user["id"], user["id"], get_session_user_id())
