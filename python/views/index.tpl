...
<div class="row panel panel-primary" id="prof">
  ...
  <div class="col-md-4">
    <dl>
      ...
      <dt>友だちの人数</dt><dd id="prof-friends"><a href="/friends">{{num_friends}}人</a></dd>
    </dl>
  </div>
  <div class="col-md-4">
  ...
  <div id="comments">
    % for comment in comments_for_me:
      <div class="comments-comment">
        <ul class="list-group">
          <li class="list-group-item comment-owner"><a href="/profile/{{comment["user_account_name"]}}">{{comment["user_nick_name"]}}さん</a>:
...
<div class="col-md-4">
  ...
  <div id="friend-entries">
    ...
    <div class="friend-entry">
      <ul class="list-group">
        <li class="list-group-item entry-owner"><a href="/diary/entries/{{entry["owner_account_name"]}}">{{entry["owner_nick_name"]}}さん</a>:
    ...
    <div class="friend-comment">
      <ul class="list-group">
        <li class="list-group-item comment-from-to">
          <a href="/profile/{{comment["comment_owner_account_name"]}}">{{comment["comment_owner_nick_name"]}}さん</a>から
          <a href="/profile/{{comment["entry_owner_account_name"]}}">{{comment["entry_owner_nick_name"]}}さん</a>へのコメント:
        <li class="list-group-item comment-comment">{{comment["comment"][:27] + '...' if len(comment["comment"]) > 30 else comment["comment"]}}
        <li class="list-group-item comment-created-at">投稿時刻:{{comment["created_at"]}}
      </ul>
    </div>
